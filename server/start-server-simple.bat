@echo off
REM Live Edits Server - Simple Robust Version
REM This version is designed to work reliably with Task Scheduler
...
REM Method 3: Try just "node" command
where node >nul 2>&1
if %ERRORLEVEL% EQU 0 (
...
echo [%DATE% %TIME%] Checked locations: >> "%ERROR_LOG%"
echo   C:\Program Files\nodejs\node.exe >> "%ERROR_LOG%"
echo   C:\Program Files (x86)\nodejs\node.exe >> "%ERROR_LOG%"
echo   PATH environment variable >> "%ERROR_LOG%"
...
start /b "" "%NODE_EXE%" src\index.js >> "%LOG_FILE%" 2>&1
...
if %ERRORLEVEL% EQU 0 (
    echo [%DATE% %TIME%] SUCCESS: Server process started >> "%LOG_FILE%"
    echo [%DATE% %TIME%] Server is running >> "%LOG_FILE%"
) else (
    echo [%DATE% %TIME%] WARNING: Node.js process not detected >> "%LOG_FILE%"
    echo [%DATE% %TIME%] WARNING: Check server.log for errors >> "%ERROR_LOG%"
)

echo [%DATE% %TIME%] Batch file completed >> "%LOG_FILE%"
exit /b 0
