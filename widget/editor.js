...
(function() {
  ...
  let state = {
    ...
    currentEditableIndex: -1, // Track current editable element index
    editableElements: [], // Array of all editable elements
  ...
  function createToolbar() {
    ...
    toolbar.innerHTML = `
      <style>
        ...
        .wysiwyg-controls button:hover:not(:disabled) {
          background: #f3f4f6;
        }
        .wysiwyg-controls button:disabled {
          opacity: 0.5;
          cursor: not-allowed;
          background: #f9fafb;
        }
    ...
    if (!wysiwygContainer) {
      ...
      wysiwygContainer.style.cssText = 'position: fixed; top: 75px; left: 0; right: 0; z-index: 9999; display: none;box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);border-bottom: 1px solid #e5e7eb;background:white;height:50px;';
  ...
  function enableEditableElements() {
    ...
    // Filter out SVG elements and store the list
    state.editableElements = elementsToEnable.filter(el => {
    ...
    state.editableElements.forEach(el => {
    ...
    // Show wysiwyg controls toolbar when edit mode is enabled
    showWysiwygControls();
  }
  ...
  function handleEditableClick(e) {
    ...
    // Find the index of this element in the editable elements array
    const index = state.editableElements.indexOf(element);
    if (index !== -1) {
      state.currentEditableIndex = index;
    }
  ...
  function showWysiwygControls(element = null) {
    ...
    if (!container) {
      ...
      container.style.cssText = 'position: fixed; top: 75px; left: 0; right: 0; z-index: 9999; display: none;box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);border-bottom: 1px solid #e5e7eb;background:white;height:50px;';
    ...
    // Get current element or use provided element
    const currentElement = element || state.currentEditableElement;
    const currentIndex = currentElement ? state.editableElements.indexOf(currentElement) : state.currentEditableIndex;
    const totalElements = state.editableElements.length;
    ...
    if (totalElements > 0) {
      ...
      const nextDisabled = currentIndex >= totalElements - 1 ? 'disabled' : '';
      const counterText = totalElements > 0 ? ` (${currentIndex + 1}/${totalElements})` : '';
      
      navButtonsHTML = `
        <button id="prev-editable-btn" ${prevDisabled} title="Previous editable section" style="margin-right: 8px;">◀ Prev</button>
        <button id="next-editable-btn" ${nextDisabled} title="Next editable section" style="margin-right: 8px;">Next ▶</button>
        <span style="margin-right: 12px; color: #6b7280; font-size: 14px;">${counterText}</span>
      `;
    }
    ...
    const prevBtn = container.querySelector('#prev-editable-btn');
    const nextBtn = container.querySelector('#next-editable-btn');
    
    if (prevBtn) {
      prevBtn.addEventListener('click', (e) => {
        e.preventDefault();
        navigateToEditable(-1);
      });
    }
    ...
    controls.querySelectorAll('button[data-command]').forEach(btn => {
  ...
  function addWysiwygControls(element) {
    showWysiwygControls(element);
  }

  function navigateToEditable(direction) {
    if (state.editableElements.length === 0) return;
    
    // Determine target index
    let targetIndex;
    if (state.currentEditableIndex === -1) {
      // No current selection, start at first or last
      targetIndex = direction > 0 ? 0 : state.editableElements.length - 1;
    } else {
      targetIndex = state.currentEditableIndex + direction;
      // Clamp to valid range
      targetIndex = Math.max(0, Math.min(targetIndex, state.editableElements.length - 1));
    }
    
    const targetElement = state.editableElements[targetIndex];
    if (!targetElement) return;
    
    // Remove editing state from current element
    if (state.currentEditableElement) {
      state.currentEditableElement.contentEditable = 'false';
      state.currentEditableElement.classList.remove('editing');
    }
    
    // Set new current element
    state.currentEditableElement = targetElement;
    state.currentEditableIndex = targetIndex;
    
    // Enable editing on target element
    ...
    setTimeout(() => {
      targetElement.focus();
      // Update controls to reflect new selection
      showWysiwygControls(targetElement);
    }, 100);
  }
  ...
  function setupEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 's') {
        // Only save if edit mode is enabled
        if (state.isEditMode) {
          e.preventDefault();
          saveChanges();
        }
  ...
  async function saveChanges() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Keep edit mode enabled after saving (user can continue editing)
      } else {
